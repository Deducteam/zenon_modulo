     ...
     scrand (ce1, ce2,
	     ...
	     not_phi_to_phi
	       ...
	       (sccut (enot ce,
		       ...
		       sclweak (enot ce2, proof))))
  | Eor (e1, e2, _) ->
     let e = enot (ctrexpr_psi e) in
     scrnot (e,
	     sccut (enot (enot e),
		    scrnot (enot e,
			    sclnot (e,
			 ...
			 scrnot (ce,
				 sclnot (ce2,
					 sclimply (ce1, ce2,
						   scaxiom (ce1, gamma),
						   scaxiom (ce2, ce1 :: gamma)))),
			 sclweak (ce1, (sclweak (enot ce2, proof))))))
  | Eall (e1, s, e2, _) ->
     let z = new_tau () in
     ...
     scrall (ctrexpr_phi e, z,
	     not_phi_to_phi
	       ez
	       (let e = ctrexpr_phi e in
		let ez = ctrexpr_phi ez in
		sccut (enot e,
		       scrnot (e,
			       sclnot (ez,
...
let rec not_psi_to_phi e proof =
  ...
  | Eimply (e1, e2, _) ->
     ...
     scrimply (ctrexpr_chi e1, ctrexpr_phi e2,
	       not_psi_to_phi
		 e2
		 (let e1 = ctrexpr_chi e1 in
		  let e2 = ctrexpr_psi e2 in
		  sccut (enot ce,
			 scrnot (ce,
				 sclnot (e2,
					 sclimply (e1, e2,
						   scaxiom (e1, gamma),
						   scaxiom (e2, e1 :: gamma)))),
			 sclweak (e1, (sclweak (enot e2, proof))))))
  | Eall (e1, s, e2, _) ->
     let z = new_tau () in
     let ez = substitute [(e1, z)] e2 in
     scrall (ctrexpr_phi e, z,
