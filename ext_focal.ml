...
let bool_to_prop_args args ty =
  if ty == type_none then earrow (List.map get_type args) t_prop
...
*)
  ...
  | Enot (Eapp (Evar("Is_true",_), [Eapp (Evar(s,_) as var, args, _)], _), _) ->
      let vssty = bool_to_prop_args args (get_type var) in
      let branches = [| [enot (eapp (tvar ("Is_true**" ^ s) vssty, args))] |] in
      [ Node {
          nconc = [e];
          nrule = Ext ("focal", "merge", []);
          nprio = Arity;
...
let to_llargs tr_expr r =
  ...
  | Ext (_, "false", []) ->
      let c = tr_expr (istrue (tvar "false" bool1)) in
      ("zenon_focal_false", [], [c], []);
  | Ext (_, "nottrue", []) ->
      let c = tr_expr (enot (istrue (tvar "true" bool1))) in
